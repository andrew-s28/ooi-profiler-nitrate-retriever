...
def _filter_dates(nc_files: list, start_date: str, end_date: str) -> list:
    ...
    nc_files_start_dates = [
        datetime.strptime(re.search(date_regex, f).group(0).split("-")[0], "%Y%m%dT%H%M%S.%f") for f in nc_files
    ]
    nc_files_end_dates = [
        datetime.strptime(re.search(date_regex, f).group(0).split("-")[1], "%Y%m%dT%H%M%S.%f") for f in nc_files
    ]
...
def _comput_tsrho(ds: xr.Dataset) -> xr.Dataset:
    ...
    ds["absolute_salinity"] = gsw.conversions.SA_from_SP(
        SP=ds["practical_salinity"], p=ds["pressure"], lon=ds["lon"], lat=ds["lat"]
    )
    ...
    ds["conservative_temperature"] = gsw.conversions.CT_from_t(
        SA=ds["absolute_salinity"], t=ds["temperature"], p=ds["pressure"]
    )
    ...
    ds["sigma_t"] = (
        gsw.density.rho(SA=ds["absolute_salinity"], CT=ds["conservative_temperature"], p=ds["pressure"]) - 1000
    )
    ds["sigma_t"].attrs = {
    ...
    ds["sigma_theta"] = gsw.density.sigma0(SA=ds["absolute_salinity"], CT=ds["conservative_temperature"])
...
def _nutnr_qc(
    ...
    with warnings.catch_warnings():
        ...
        wavelengths = np.arange(190, 394.01, 0.8)[30:]
        spectral_channels = (spectral_channels.T - dark_val).T[:, 30:]
        mask = np.full(time.shape, True, dtype=bool)
        a_all = np.empty(len(time))
        a_all[:] = np.nan
        step_0, step_1, step_2, step_3, step_4 = 0, 0, 0, 0, 0

        mask[qartod != 1] = False
        step_0 = np.count_nonzero(~mask)

        # mask observations with low salinity values
        mask[(salinity <= min_salinity) & (mask)] = False
        step_1 = np.count_nonzero(~mask) - step_0
        ...
        step_2 = np.count_nonzero(~mask) - step_0 - step_1
        ...
        for i, sp in enumerate(spec_fit):
            if np.any(~np.isfinite(sp)):
                if mask[i]:
                    ...
                    )  # any non-finite values should have been removed in step 2
                    mask[i] = False
                    step_2 += 1
            else:
        ...
        step_4 = np.count_nonzero(~mask) - step_0 - step_1 - step_2 - step_3
    return mask, a_all, step_0, step_1, step_2, step_3, step_4


def _nutnr_qc_wrapper(ds: xr.Dataset, min_salinity: float, rmse_lim: float = 0.02) -> xr.Dataset:
    """
    ...
    mask, a_all, step_0, step_1, step_2, step_3, step_4 = _nutnr_qc(
        ds.spectral_channels.values,
        ds.dark_val.values,
        ds.practical_salinity.values,
        ds.time.values,
        ds.salinity_corrected_nitrate_qartod_results.values,
        min_salinity,
        rmse_lim=rmse_lim,
    )
    # mask the data
    ...
    return ds, np.count_nonzero(mask), step_0, step_1, step_2, step_3, step_4, removed
...
def _parse_args() -> tuple[str, str, str, str]:
    ...
    parser.add_argument(
        "site", metavar="site", type=str, nargs="+", help="Site to download from, either CE01ISSP or CE02SHSP"
    )
    parser.add_argument(
        "-p",
        "--path",
...
def _get_datasets(site: str, start_date: str, end_date: str) -> list:
    if site == "CE01ISSP":
        ...
        raise ValueError(
            "Invalid site selection, choose one or more of CE01ISSP (Oregon inshore) or CE02SHSP (Oregon midshelf)."
        )
...
if __name__ == "__main__":
    ...
    for site in sites:
        ...
        ds_qced, deployment_numbers, total_data, good_data, step_0, step_1, step_2, step_3, step_4, removed = (
            [],
            [],
            [],
            [],
        ...
        for _i, d in enumerate(tq(ds, desc="QC'ing datasets")):
            deployment_numbers.append(np.unique(d.deployment)[0])
            total_data.append(d.time.size)
            out = _nutnr_qc_wrapper(d, min_salinity=25)
            if out[0] is not None:
                ds_qced.append(out[0])
            ...
            step_3.append(out[5])
            step_4.append(out[6])
            removed.append(out[7])

        for rem in removed:
            if rem:
                print(f"Deployment {removed} removed due to lack of data after QC.")

        qc_results = pd.DataFrame(
            {
                "deployment": deployment_numbers,
        ...
        for i, d in enumerate(deployments):
            if d.time.size < 10:
                deployments.pop(i)
                print(
                    f"Deployment {d.deployment.mean().values} removed due to having less than 10 profiles after QC and binning."
                )
        ...
        if site == "CE01ISSP":
            ...
            for i, dep in enumerate(deployments):
                d = np.unique(dep.deployment)[0]
                baseline = dep.isel(depth=slice(0, 2)).salinity_corrected_nitrate.median()
                # the expected accuracy of the SUNA is +/- 3 mmol/m^3
                # for some deployments that have many high nitrate events at the surface (consistent upwelling?), the median is too large
                # in these cases, we adjust the baseline to ensure that the minimum value is at least -3 mmol/m^3
                # deployments which don't have this issue are not adjusted to avoid the opposite problem
                if (dep["salinity_corrected_nitrate"] - baseline).min() < -3:
                    baseline += (dep["salinity_corrected_nitrate"] - baseline).min() + 3
                deployments[i]["salinity_corrected_nitrate"] = dep["salinity_corrected_nitrate"] - baseline
        elif site == "CE07SHSP":
            for i, dep in enumerate(deployments):
                d = np.unique(dep.deployment)[0]
        ...
        if site == "CE01ISSP":
            ds_bin_baseline_subtracted = ds_bin_baseline_subtracted.where(
                ds_bin_baseline_subtracted.deployment != 20, drop=True
            )
        elif site == "CE06ISSP":
            ds_bin_baseline_subtracted = ds_bin_baseline_subtracted.where(
                ds_bin_baseline_subtracted.deployment != 15, drop=True
            )
